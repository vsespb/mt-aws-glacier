...
BEGIN { *CORE::GLOBAL::open = sub(*;$@) { push @$OpenStack, \@_; CORE::open($_[0], $_[1], $_[2]) }; };
...
ok open_file($tmp_file, mode => '>', binary => 1);
...
new_stack {
	ok open_file($tmp_file, mode => '>', file_encoding => 'UTF-8');
...
new_stack {
	ok open_file($tmp_file, mode => '>', file_encoding => 'KOI8-R');
...

{
	...
	ok my $f = open_file($tmp_file, mode => '<', file_encoding => 'UTF-8');
	my $line = <$f>;
	is $line, 'тест';
	unlink $tmp_file;
}

{
	...
	ok my $f = open_file($tmp_file, mode => '<', binary => 1);
	my $line = <$f>;
	is $line, $encoded;
	unlink $tmp_file;
}
...
new_stack {
	...
	eval { open_file($utfname, mode => '>', binary => 1); };
	is last_call->[2], encode("UTF-8", $utfname), "should use filename_ecnoding by default";
};

new_stack {
	my $utfname = $mtroot."/тест";
	eval { open_file($utfname, mode => '>', binary => 1, use_filename_encoding => 1); };
...
new_stack {
	my $utfname = $mtroot."/тест";
	eval { open_file($utfname, mode => '>', binary => 1, use_filename_encoding => 0); };
